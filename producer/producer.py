...
channel_update = connection.channel()
...
channel_update.queue_declare(queue='update_data', durable=True)
...
def routes():
    message = {
        ...
        "to update data": "http://127.0.0.1:5050/update-record"
    }
...
def update_record():
    if request.method == "POST":
        ...
        message = {"srn": srn, "name": name, "section": section}
        channel_update.basic_publish(
            exchange='',
            routing_key='update_data',
            body=json.dumps(message),
            properties=pika.BasicProperties(delivery_mode=pika.spec.PERSISTENT_DELIVERY_MODE)
        )
